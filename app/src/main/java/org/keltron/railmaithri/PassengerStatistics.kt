...
class PassengerStatistics: AppCompatActivity() {
    ...
    private lateinit var densitys: JSONArray
    ...
    private fun validateInput(): JSONObject? {
        ...
        val densityNumber = densitys.getJSONObject(densityPos).getString("id").toString()
        val compartmentTypePos = compartmentTypeSP.selectedItemPosition
        val compartmentTypeNumber =
            compartmentTypes.getJSONObject(compartmentTypePos).getString("id").toString()
    ...
    private fun sendForm(formData: JSONObject) {
        try {
            ...
            val request =
                API.postRequest(token!!, API.PASSENGER_STATISTICS, formData,file = null,fileName = null)
